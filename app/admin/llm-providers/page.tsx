    ...
    <div className="space-y-8 text-[13px] text-[var(--text-dim)]">
      <div>
        ...
        <p className="mt-2 text-[12px] text-[var(--text-dim)]">
          Backed by <code className="text-[var(--accent)]">llm_providers</code>. Use JSON payloads to manage provider
          metadata and API config.
        </p>
      </div>
      ...
      <section className="border border-[var(--border)] bg-[var(--bg-surface)] rounded-[4px] p-5">
        <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-[12px] text-[var(--text-dim)]">
              ...
              <code className="text-[var(--accent)]">llm_providers</code>.
            </p>
          </div>
          <div className="text-[11px] text-[var(--text-muted)]">
            Tip: keep secrets in env vars; store only identifiers here.
          </div>
        </div>
        <form action={createRow} className="mt-4 space-y-3">
          ...
          <textarea
            ...
            placeholder='{"name":"OpenAI","slug":"openai","base_url":"https://api.openai.com",...}'
          />
