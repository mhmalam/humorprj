    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-2 text-sm text-white/70">
          Reading from <code className="text-white/85">llm_model_responses</code>. Inspect how your models are actually
          responding in production.
        </p>
      </div>
      ...
      <section className="rounded-[24px] border border-white/10 bg-white/[0.06] backdrop-blur-xl overflow-hidden shadow-[0_24px_90px_-60px_rgba(0,0,0,0.8)]">
        <div className="px-6 py-4 border-b border-white/10 flex items-center justify-between gap-4">
          <div className="text-sm font-semibold text-white/85">Responses</div>
              ...
              <tbody>
                {rows.map((r) => (
                  <tr
                    ...
                    className="border-t border-white/10 hover:bg-white/[0_0_0_0.8)] transition"
                  >
                    {columns.map((c) => (
                ...
                {rows.length === 0 ? (
                  <tr>
                    <td className="px-4 py-6 text-white/60" colSpan={columns.length || 1}>
                      No llm_model_responses rows found.
                    </td>
                  </tr>
                ) : null}
              </tbody>
